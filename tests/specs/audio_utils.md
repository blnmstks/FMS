...
## `pad_wav_to_duration(input_path: str, output_path: str, target_seconds: float) -> str`

### Contract
Дописывает тишину в ХВОСТ WAV до `target_seconds` и пишет результат в `output_path`
(формат wav, pydub). Если входной файл уже не короче цели — копирует как есть (не режет!).
Создаёт родительские директории. Возвращает `output_path`.

Зачем (шаг 13): длительность видео снапится вверх до `8n+1` кадров (`ltx_snap_duration`),
а нода TrimAudioDuration в workflow умеет только резать — аудио-латент оказался бы короче
видео-латента. Паддим WAV до snap-длительности перед загрузкой: A/V-латенты совпадают,
речь никогда не обрезается.

### Invariants
1. Длительность результата ≈ `max(вход, target_seconds)` (точность pydub — целые мс).
2. Исходные сэмплы не меняются (тишина только дописывается в конец); вход не короче цели →
   контент идентичен входу.
3. Создаёт `output_path.parent`; возвращает `output_path`.

### Test cases (unit, `tmp_path`, реальные мини-WAV через `write_wav`)
- **паддинг**: WAV 0.1 с → `pad_wav_to_duration(src, dst, 0.3)` → длительность `dst`
  ≈ 0.3 с (±5 мс).
- **уже длиннее цели**: WAV 0.1 с, цель 0.05 → длительность `dst` ≈ 0.1 с (не порезан).
- **создаёт вложенные папки** и **возвращает путь**.
